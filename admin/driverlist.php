...
$q = "select id, RegNo, DriverName, DateOfJoin, email from adddriver";

$result = mysqli_query($con,$q);

if($result-> num_rows > 0){
    while($row = $result-> fetch_assoc()){
        echo "<tr><td>". $row["id"] ."</td><td>".$row["RegNo"]."</td><td>".$row["DriverName"]."</td><td>".$row["DateOfJoin"]."</td><td>".$row["email"]."</td></tr>";
    }
    echo "</table>";
}
else{
